            ...
            <form action="{{ route('admin.updateuserrr', $users->id) }}" method="POST" class="space-y-4">
                ...
                <div class="flex items-center space-x-4" id="unitPendidikanContainer" style="display: {{ old('role', $users->role) == 'tuunit' ? 'flex' : 'none' }};">
                    ...
                    <select id="nama_unit" name="nama_unit" class="w-3/4 px-4 py-2 border rounded-md">
                        ...
                        <option value="TK" {{ old('nama_unit', $users->nama_unit) == 'TK' ? 'selected' : '' }}>TK</option>
                        <option value="SD" {{ old('nama_unit', $users->nama_unit) == 'SD' ? 'selected' : '' }}>SD</option>
                        <option value="SMP" {{ old('nama_unit', $users->nama_unit) == 'SMP' ? 'selected' : '' }}>SMP</option>
                        <option value="SMA" {{ old('nama_unit', $users->nama_unit) == 'SMA' ? 'selected' : '' }}>SMA</option>
                        <option value="TPQ" {{ old('nama_unit', $users->nama_unit) == 'TPQ' ? 'selected' : '' }}>TPQ</option>
                        <option value="PONDOK" {{ old('nama_unit', $users->nama_unit) == 'PONDOK' ? 'selected' : '' }}>PONDOK</option>
                        <option value="MADIN" {{ old('nama_unit', $users->nama_unit) == 'MADIN' ? 'selected' : '' }}>MADIN</option>
                    </select>
                </div>
